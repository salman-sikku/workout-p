...
const TopTrainInstruction: React.FC<ParamsProps> = ({ paramsId }) => {
    ...
    useEffect(() => {
        const fetchDataByWorkout = async () => {
            console.log('i am run to hare')
            try {
                const { data } = await axios.get(`/api/train-exercise/singleTrainExer/${paramsId}`);
                console.log(data.getSingleTrainExso);
                setSingleChalengExso(data.getSingleTrainExso);
            } catch (error) {
                console.log(error);
            }
        };
...
export default TopTrainInstruction;
