...
const ShowTrainingExsoList: React.FC<ParamsProps> = ({ paramsId }) => {
    ...
    useEffect(() => {
        const fetchDataByCat = async () => {
            try {
                const { data } = await axios.get(`/api/train-exercise/trainByCate/${paramsId}`);
                setCategoryData(data.category);
                setWorkoutList(data.TrainCategoryExso);
                setLoading(false);
            } catch (error) {
                console.log(error);
                setLoading(false);
            ...
            <div className='mt-7 sm:mt-8 md:mt-10 px-1 sm:px-2 md:px-4 mb-[82px] sm:mb-[85px] md:mb-[98px]'>
                {
                    loading ? (
                        ...
                        workoutList.map((curElm: any, index: number) => (
                            <Link href={`/singleTrainWork/${curElm._id}`} className='mt-[5px] md:mt-4 sm:mt-2' key={index}>
...
export default ShowTrainingExsoList;
